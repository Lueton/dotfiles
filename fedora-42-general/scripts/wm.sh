...
sudo dnf install -y sway swaybg swaylock hyprlock swayidle waybar mako wofi grim slurp wl-clipboard nemo
...
# hyprlock config is regenerated by pywal on each 'wallpaper' call — copy static fallback for first boot
mkdir -p "$HOME/.config/hypr"
if [ ! -f "$HOME/.config/hypr/hyprlock.conf" ]; then
    cp "$DOTFILES_DIR/config/hypr/hyprlock.conf" "$HOME/.config/hypr/hyprlock.conf"
fi
